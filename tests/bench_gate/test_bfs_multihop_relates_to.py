"""Bench gate for #383 — Track A `RELATES_TO` edge type ship gate.
...
`RELATES_TO` ships only when it demonstrates **≥+5pp BFS multi-hop hit@k
uplift** on the labeled fixture vs. the same fixture run with
`BFS_EDGE_WEIGHTS[RELATES_TO]` zeroed (which causes the BFS expander to
skip RELATES_TO edges per `bfs_multihop.py:155-160`).

Skips cleanly when `AELFRICE_CORPUS_ROOT` is unset (public CI), when the
`bfs_relates_to/` module dir is missing, or when the corpus has fewer than
...
def _build_store(tmp_path: Path, row: dict, arm: str):
    ...
    `arm` namespaces the db path so the two arms (with/without RELATES_TO)
...
def test_relates_to_edge_uplift(
    aelfrice_corpus_root: Path,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    rows = [
        r for r in load_corpus_module(aelfrice_corpus_root, "bfs_relates_to")
        if not r.get("seed", False)
    ]

    if len(rows) < MIN_ROWS:
        pytest.skip(
            f"bfs_relates_to corpus has {len(rows)} non-seed rows; gate requires "
    ...
    # Arm 1: full edge weights (RELATES_TO at its production weight 0.30).
    with_hits = 0
    for row in rows:
    ...
    # Arm 2: zero out RELATES_TO. The BFS expander treats edge_w == 0.0 as
    # "skip" without marking visited, so this isolates the edge's
    # contribution to reachability.
    monkeypatch.setitem(BFS_EDGE_WEIGHTS, EDGE_RELATES_TO, 0.0)
    without_hits = 0
    for row in rows:
