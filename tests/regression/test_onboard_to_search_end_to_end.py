...
# --- Inserted beliefs interact with feedback loop ----------------------


def test_scanned_belief_can_be_locked_and_unlocked_by_demotion(
    tmp_path: Path,
) -> None:
    """Cross-milestone integration: a scanned belief, once locked, can
    be auto-demoted via the v0.4.0 feedback loop. Single test scenario,
    not a substitute for the v0.4.0 feedback regression which covers
    the demotion mechanics in detail."""
    ...
    # Add an explicit contradictor (would normally come from later scan).
    contradictor = Belief(
        id="X",
        content="we do not ship the regex fallback at all",
        content_hash="h_X",
        alpha=1.0,
        beta=1.0,
        type=BELIEF_FACTUAL,
    ...
    s.insert_belief(contradictor)
    s.insert_edge(Edge(src="X", dst=target.id, type=EDGE_CONTRADICTS, weight=1.0))

    for _ in range(DEMOTION_THRESHOLD):
        apply_feedback(s, "X", valence=1.0, source="user")

    after = s.get_belief(target.id)
