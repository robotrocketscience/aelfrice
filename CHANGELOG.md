...
- **`/aelf:upgrade` slash uses the no-flag `aelf upgrade-cmd` form for cross-CLI compatibility** ([#530](https://github.com/robotrocketscience/aelfrice/issues/530)). The v2.0.1 slash file invokes `aelf upgrade-cmd --check` and parses `run:` out of the output, but v2.0.1's CLI suppresses `run:` under `--check` (the #522 fix is unreleased). On v2.0.1 installs the slash silently does nothing — the user reads "(installed: <old>)" out of the partial output and concludes the upgrade did not happen. Step 1 now invokes the no-flag form, which has emitted `run:` on every released CLI back to v1.0.1, making the slash robust against the same flag-gated regression class. Companion to #522 (CLI symmetry); #522 prevents recurrence post-v2.0.2, this prevents the parse-step blackout for any pre-#522 user who lands the new slash via `aelf setup`. v2.0.1 users currently stranded should run their install-method's upgrade command directly (`pipx upgrade aelfrice`, `uv tool upgrade aelfrice`, etc.) and then `aelf setup` to redeploy the fixed slash bundle.
