...
- **CI wired for GitHub native merge queue** ([#624](https://github.com/robotrocketscience/aelfrice/issues/624)). `.github/workflows/ci.yml` and `.github/workflows/staging-gate.yml` now also trigger on `merge_group` events so the five required checks (`pytest (3.12)`, `pytest (3.13)`, `secrets-scan`, `pattern-scan`, `history-scan`) re-run on the rebased `gh-readonly-queue/main/pr-N-<sha>` branch GitHub builds for each queue entry. The four PR-context-only staging-gate jobs (`release-docs-check`, `commit-msg-prefix`, `pr-title-prefix`, `pr-body-issue-link`) are gated to `pull_request` events via `if:` since their event-payload inputs don't exist on `merge_group`. `history-scan` now reads its base/head from `github.event.merge_group.{base_sha,head_sha}` when fired from the queue (the queue branch is already rebased onto `base_sha`, so the incoming range is `base_sha..head_sha` exactly). Closes the structural failure of `merge-train.yml`'s FF-only model under parallel sessions, where the second-and-later labeled PRs always bounced because main moved between rebase and slot; see #624 for the full diagnosis. The legacy `merge-train.yml` stays in place this PR as fallback and will be removed once the queue is verified merging cleanly. Admin-only ruleset change required at landing: add a `merge_queue` rule to the Default ruleset on `refs/heads/main` with `merge_method: REBASE` and `grouping_strategy: ALLGREEN`.
