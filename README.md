...
| v1.7 | shipped (opt-in) | graph signal wave + structural retrieval lane — signed Laplacian + eigenbasis (#149), heat kernel authority (#150), Plate FFT HRR primitives (#216), HRR bind/probe (#152), `uri_baki` post-rank adjuster retest (#153). Components ship behind opt-in feature flags in `[retrieval]` of `.aelfrice.toml` (`use_signed_laplacian`, `use_heat_kernel`, `use_hrr_structural`). Default-on flip (#154) deferred — bench harness (#403/#425) showed `use_bm25f_anchors` +0.6010 NDCG@k uplift on the v0.1 retrieve_uplift fixture but uncovered a stemming gap vs. FTS5 (`banana` vs `bananas` mismatch); flip blocked on stemming evidence. |
| v2.0 | planned | feature parity with the original research line + benchmark reproducibility. v2.0's component issues land incrementally across v1.5–v1.7; final v2.0 tag is the reproducibility cut. v1.7 default-on flip is a prereq; deferred per #154. |
