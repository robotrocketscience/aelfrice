...
| **v0.2.0** | ✅ shipped (`b32491d`) | `scoring.py` (Beta-Bernoulli posterior mean, type-specific decay with lock-floor short-circuit, basic relevance combiner) + property-derived tests for Bayesian inertia ([redacted]), decay-required ([redacted]), and lock-floor sharpness ([redacted]) |
...
After v1.0.0, the `v1.x` series recovers v2.0 features incrementally with R&D justification per `[redacted]` deferral schedule (HRR retrieval → v1.1; entity-index → v1.2; lock tiers `promoted`/`freeze` → v1.3; LLM classification → v1.4; anneal maintenance → v1.5; etc.).
...
These constrain every commit. See [`[redacted]`](https://github.com/robotrocketscience/[redacted]) for the full list. Highlights:
...
Rebuild driven by earlier work 1-6 (24 experiments) and the [project scope contract](https://github.com/robotrocketscience/[redacted]). The legacy v2.0 codebase remains accessible at [`aelfrice-v0`](https://github.com/robotrocketscience/aelfrice-v0) (archived).
