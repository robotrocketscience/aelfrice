...
- **Passive capture.** Eight default-on hooks: `UserPromptSubmit` retrieval, four-event transcript-ingest, `PostToolUse:Bash` commit-ingest, `SessionStart` locked-belief injection (with a belief-write recap line, v3.5+), `Stop` lock-prompt, `PreToolUse:Grep|Glob` memory-first search, `PreToolUse:Bash` memory-first search, and the `PreToolUse:Bash` issue-dup guard. Session activity flows into the belief graph without you typing `aelf` at all; opt out per-hook — see [INSTALL § default-on hooks](docs/user/INSTALL.md).
