...
2. **No topic / flag-only** (`/aelf:wonder` with no args, or only flags like `--top 5`, `--emit-phantoms`, `--seed <id>`, `--persist`) **→ graph-walk consolidation** (with `--gc`, instead runs the wonder GC pass that soft-deletes stale speculative beliefs). Walk the belief graph from a deterministically-picked seed (or `--seed <id>`) and surface ranked consolidation candidates with suggested actions. Use `--emit-phantoms` to print Phantom JSON for offline review or `--persist` to write them to the store via `wonder_ingest`.
</objective>
...
**Automatic GC (opt-in, default-off — #980).** `--gc` is manual-only by default, which is why phantoms accumulate forever (the #980 audit found 0 GC'd across every store, ever). Set `AELFRICE_WONDER_AUTOGC=1` to run `wonder_gc` once per session on the SessionStart hook so stale phantoms (≥14 days old, unchanged priors, no feedback, no RESOLVES edge) converge to soft-deleted instead of piling up. Override the age threshold with `AELFRICE_WONDER_AUTOGC_TTL_DAYS=<days>` (default 14, min 1). Each automatic sweep that deletes anything emits a `wonder.gc` row to the feed log (visible in `/aelf:feed` and the `/aelf:status` phantom-lifecycle line) plus a one-line `aelf-hook:` notice on stderr. It never writes to the injected context block. Leave the flag unset to keep GC strictly manual.
</process>
