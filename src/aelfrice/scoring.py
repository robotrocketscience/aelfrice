...
This module has no posterior decay. The half-life table, the type
half-life lookup and the Jeffreys-targeted ager were removed in #1369
(under #1162): nothing under `src/` had ever called them, so nothing
has ever moved a stored `(alpha, beta)` toward the prior, and the lock
short-circuit inside the ager exempted locks from a mechanism that did
not run. Two live mechanisms keep the word *decay*, and neither touches
a stored posterior: `retrieval._apply_temporal_decay` discounts a
*ranking* score by age (reachable only through `retrieve_v2` behind
`temporal_sort`, which defaults to off), and
`meta_beliefs.decay_toward_default` ages a *meta-belief* toward its
static default.
