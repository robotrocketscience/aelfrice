    ...
    *,
    ...
    try:
        ...
        if hits:
            ...
            #
            # **It is the CALLER's set on purpose, and #1564 tested the
            # alternative.** An adversarial read of this call site said the
            # set should be the renderer's instead -- `already_rendered`
            # unioned with the session-start sub-block's own ids, which is
            # what `_format_hits_with_session_start` dedupes against --
            # because the two derivations differ and #1382 AC4 asks for
            # one. They differ on exactly the hits the sub-block rendered
            # verbatim above the pack, and those are the hits whose text IS
            # in the window: the sub-block put it there. Recording them is
            # the true claim, and the union would drop it. Fired on an
            # 80-lock / 20-core store with `AELFRICE_TURN_DIFFERENTIAL=1`,
            # the union takes turn two's `seen` pointers to zero and the
            # feature stops working across turns --
            # `test_ups_seen_pointer_on_turn_two_names_a_belief_turn_one_rendered`
            # in `test_hook_injection_ceiling_wiring.py` is the disproof,
            # and `test_the_ledger_keeps_a_belief_the_sub_block_rendered`
