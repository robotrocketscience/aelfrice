...
- **One global DB shared across every project on the machine.** The default DB resolves to `~/.aelfrice/memory.db` (`src/aelfrice/cli.py::db_path()`); `mcp_server.py` and `hook.py` both delegate to the same resolver. There is no per-project keying — onboarding repo A and repo B writes both projects' beliefs into one file. Workaround today: pin per-project with `AELFRICE_DB=/abs/path/.aelfrice.db` (e.g. via direnv). v1.1.0 lands `.git/aelfrice/memory.db` as the default for git repos (keyed off the git-common-dir so worktrees of the same repo share one DB), with `~/.aelfrice/memory.db` as the non-git fallback, plus a one-shot `aelf migrate` that copies beliefs from the legacy global store into the active project's in-repo store, plus worktree concurrency tests. `.git/` is not git-tracked, so the brain graph never crosses the git boundary. Note that fresh clones intentionally start with an empty store — the brain graph stays on the machine it was written on (see [§ Sharing or sync of brain-graph content](#sharing-or-sync-of-brain-graph-content)). Bootstrap a clone with `aelf onboard .`.
