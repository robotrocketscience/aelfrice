...
- **`aelf onboard` is non-incremental on duplicates.** Re-runs are idempotent; existing beliefs are not re-scored or refreshed. (v2.x #264: every ingest entry point now writes to `ingest_log` and materialises `beliefs` via the derivation worker, so a future `aelf rebuild --rule-set <hash>` (#265) can re-derive the canonical store from the log without a re-onboard. Today, `aelf doctor --derive-pending` runs the worker once over any unstamped log rows — useful when a prior batch crashed mid-stamp.)
