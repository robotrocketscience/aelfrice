...
This project is a rebuild, not a port. The rebuild fixed the structural issues that survived the research line; it fixed them at the foundation layer. A test or a benchmark supports every behavioural claim; items without such support have a transparent issue trail.
...
- The user-facing name `edges` becomes `threads`, though the internal schema does not change. The deprecation window covered both keys.
- Onboard git-recency weighting. `belief.created_at` is the most recent commit of the source file, so decay penalises stale branches.
...
This patch release follows v3.0.0, with no public API change. The user-facing surface narrows on the install side and broadens on the retrieval side. For per-entry detail, read [CHANGELOG/v3.md § 3.0.1](../../CHANGELOG/v3.md).
...
The four "candidate" lines are the orphaned research-line capabilities from the agentmemory parity audit. They do not ship today, and this roadmap did not list them before. They land if and only if a benchmark or an experiment justifies the inclusion, as [Validation](#validation) describes. Otherwise they stay parked.
