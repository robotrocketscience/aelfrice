...
| Determinism | None (LLM decides what to write and when) | Bit-level reproducible — write log replay reconstructs every *belief* state; **edges are excepted**, being written outside the log as shipped ([#1283](https://github.com/robotrocketscience/aelfrice/issues/1283)) |
