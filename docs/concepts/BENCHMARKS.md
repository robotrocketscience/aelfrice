...
> **`retrieve_v2` and the production hook path have converged (v4.0).** The academic-suite adapters and lane ablations call **`retrieve_v2()`**. As of v4.0 the production `UserPromptSubmit` hook path (`retrieve()`) is a thin adapter over `retrieve_v2` — the [#1107](https://github.com/robotrocketscience/aelfrice/issues/1107) cutover — and the staged lanes have graduated onto it one at a time behind their gates: **temporal spine ([#1064](https://github.com/robotrocketscience/aelfrice/issues/1064)), entity-persistence demotion ([#1096](https://github.com/robotrocketscience/aelfrice/issues/1096)), intentional clustering ([#436](https://github.com/robotrocketscience/aelfrice/issues/436)), and HRR-structural ([#152](https://github.com/robotrocketscience/aelfrice/issues/152)) are now live on production retrieval.** So uplift attributable to them (e.g. temporal spine's +14.6pp LoCoMo coverage) reflects production behaviour, not just the eval surface. Two staged lanes are **exposed on `retrieve_v2` but held OFF by default**, so they do **not** affect production and their numbers are ablation-only: origin tie-break ([#1089](https://github.com/robotrocketscience/aelfrice/issues/1089), refuted on LoCoMo [#1013](https://github.com/robotrocketscience/aelfrice/issues/1013)) and HRR-expand ([#981](https://github.com/robotrocketscience/aelfrice/issues/981), measured recall-neutral [#1001](https://github.com/robotrocketscience/aelfrice/issues/1001)). The always-on lanes (BM25F, heat-kernel, entity-index L2.5, BFS, type-aware compression, γ/ζ posterior rerank) likewise reflect production behaviour.
