...
- **Reader-independent** — `retrieval_quality.mrr` and `retrieval_quality.recall_at_k`. Every adapter that scores a blob reports these two metrics. They read the *ordering* of the retrieved list, so they do not move with the budget the way token-F1 does. To improve them, rank a relevant belief higher. That is the thing the benchmark exists to measure. **Do not read them as monotone in the budget** ([#1574](https://github.com/robotrocketscience/aelfrice/issues/1574)). This paragraph used to say a smaller budget truncates the tail and can only lower them; `clustering.pack_with_clusters` does not truncate. It skips a belief it cannot afford and keeps filling, so the budget *selects*, and one extra token can admit a dear belief that evicts several cheap ones. With the gold-bearing belief among the cheap ones, raising the budget takes both metrics from 1.0 to 0.0. Compare two budgets as two different selections, not as a list and its truncation.
