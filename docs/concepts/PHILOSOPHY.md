...
**What actually ships is narrower than that framing suggests, and the distinction matters** ([#1218](https://github.com/robotrocketscience/aelfrice/issues/1218)). There is **no posterior decay**: nothing moves a stored `(α, β)` toward the prior, ever. A `scoring.decay` implementing it — including a lock short-circuit that returned `(α, β)` unchanged regardless of age — sat in the tree from v1.0 with no caller under `src/`, so the exemption protected locks from a mechanism that did not run. [#1162](https://github.com/robotrocketscience/aelfrice/issues/1162)'s disposition was wire-it-or-delete-it, and it was **deleted** in [#1369](https://github.com/robotrocketscience/aelfrice/issues/1369): wiring it needs a calibration bench that is not funded, and leaving a plausible-looking unwired function in place is what made this claim readable as shipped behaviour in the first place.
...
- **Append-only substrate.** The research line had a separate `observations` table — insert-only, every observation that produced a belief permanently recorded. v1.x kept only `feedback_history` immutable (every `apply_feedback` writes a row, rows are never updated). v1.5 (#205) added the append-only `ingest_log` table; v1.7 (#264) routed every ingest entry point through the derivation worker, and the v2.0.1 view-flip (#265) made `beliefs` a materialized projection of the log. Beliefs are still mutated by feedback (and by lifecycle operations such as retire / restore), but the full write log is durable and replay-capable. See [`design/write-log-as-truth.md`](../design/write-log-as-truth.md) for the architectural memo.
  ...
  **`edges` are not covered by the log today, and this is the one place the substrate claim overreaches.** The contract ratified for them ([#1283](https://github.com/robotrocketscience/aelfrice/issues/1283), 2026-08-01) is *log-derived* — the edge set is to be recomputable from `ingest_log`, ordered by `(created_at, ingest_log ULID)`. That is the decision, not the current state. As shipped, all six `derive()` return paths emit `edges=[]`, so `ingest_log.derived_edge_ids` is NULL on **every** row, and every real edge is written outside the log by `ingest.py` (`DERIVED_FROM`), `temporal_spine.py` (`TEMPORAL_NEXT`), and the relationship / contradiction detectors. The practical cost: a replay from empty cannot reconstruct the L3 BFS graph, the temporal spine or the `CONTRADICTS` substrate, and the replay probe reports edge divergence in a bucket that never triggers drift. Note also that the ordering the writer actually uses is `(created_at, rowid)`, and `rowid` is implicit here — VACUUM may renumber it — which is why the ratified key is the log's ULID rather than anything read off the belief table.
