...
| `federation.py` | (v3.0+) Read-only peer-DB federation. `load_peer_deps()` parses `knowledge_deps.json`; `open_peer_connection(path)` opens a peer SQLite in `mode=ro&immutable=1`; `ForeignBeliefError` rejects mutations against foreign belief ids at the API surface. See [LIMITATIONS § Sharing, sync, or federation](../user/LIMITATIONS.md). |
