...
modules against the 131 `.py` files under `src/aelfrice/`, so it isn't an exhaustive map.
...
| `rebuild_log.py` | The `[rebuilder]` and `[rebuild_log]` configuration resolution, and the phase-1a diagnostic log. It is held apart from `context_rebuilder.py` so that neither one reaches the retrieval subtree. `hook.py` calls `load_rebuilder_config` on the prompt-shape-gate skip path, where deciding whether the log is enabled is itself the configuration read, so the split is what lets a skipping fire avoid importing the code it is skipping ([#1527](https://github.com/robotrocketscience/aelfrice/issues/1527)). It is the same remedy `sidecar_outcome.py` applies for `bm25`. `context_rebuilder.py` re-exports every name in `rebuild_log.__all__` except `_extracted_entities_for_log` and `_recent_turns_hash`, which nothing outside `rebuild_log.py` calls; `tests/test_rebuild_log_reexports_1527.py` pins that exception list and checks that every `from aelfrice.context_rebuilder import ...` in the tree still resolves. Names that were only ever imports leaking into `context_rebuilder`'s namespace (`extract_triples`, `os`, `tomllib` and nine others, listed in `rebuild_log.py`'s module docstring) did not come with them. |
