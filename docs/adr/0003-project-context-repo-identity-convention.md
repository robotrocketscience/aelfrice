...
- **Active-context resolution (decision 4 — ratified opt-in, #973, 2026-06-19):** the resolver default stays **env-driven** — `active_project_context()` returns `''` (no filter) unless `AELFRICE_PROJECT_CONTEXT` is set. Scoping is **opt-in**: export `AELFRICE_PROJECT_CONTEXT="$(repo identity)"` to activate it. The column is populated and migrate-safe but the resolver does **not** silently reverse #858's documented "unset = no filter" contract. The operator ratified keep-opt-in over the auto-consult flip in #973: for a single repo the flip is a no-op, while for a merged/multi-repo store it would silently hide differently-stamped beliefs — a behaviour change that cuts against the #605 narrow-surface philosophy. This decision is **closed**, not deferred.
...
- **Flip the resolver default to repo identity (auto-consult, decision 4 = yes).** Rejected (ratified in #973, 2026-06-19): it reverses the documented "unset = no filter" contract and changes retrieval behaviour for every store (a no-op for a single repo, but a silent semantics change that would hide differently-stamped beliefs in a merged store). The column and migrate provenance landed first in #970/PR #972; the operator then ratified keep-opt-in rather than the flip.
