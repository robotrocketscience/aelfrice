...
- **H1 split + deferred (#374).** Bench gate: ≥80% precision + ≥60% recall on a labeled sample of 200 coding prompts before reopening. That bar is a *reopening* criterion, so `tests/bench_gate/test_directive_detection.py` asserts it **inverted** (#1502): it is green while the bar is unmet, which is the shipped state, and fires when `detect_directive` clears it. Asserting the bar directly made the check red at every release cut by design, in the tier that is the project's only scheduled quality signal (#1477) — a permanently red check cannot be told from a broken one. Latest measurement, 510-row union corpus (v0.1 + v0.2): precision 0.665, recall 0.636. H1 stays deferred.
