...
| **`/tmp/...` data dirs** | locomo, structmemeval | Doc surface (`docs/user/COMMANDS.md`) explains the manual download. Harness skips with a clear "data dir missing at $PATH; see docs" message rather than failing — adapter exits 2 (skip), not 1 (fail). Aggregated result records `"status": "skipped_data_missing"`. |
...
- `README.md`: badge added; `## Reproducibility` section linking to `docs/user/COMMANDS.md` and the canonical artifact.
- `docs/user/COMMANDS.md`: `aelf bench` documented next to `aelf locked`, `aelf doctor`, etc.
- `docs/user/LIMITATIONS.md`: drops the "no end-to-end reproducibility surface" caveat (or adds it if not yet listed).
...
- Source-of-truth: [docs/concepts/ROADMAP.md § v2.0.0](../concepts/ROADMAP.md) — "Reproducibility harness: `benchmarks/results/v2.0.0.json` is canonical; CI runs the academic suite nightly."
...
7. Docs: README badge + `## Reproducibility` section, `docs/user/COMMANDS.md` entry, `docs/user/LIMITATIONS.md` caveat removal.
