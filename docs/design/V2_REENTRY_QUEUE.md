...
- **Iteration spec:** [`docs/design/v2_directive_detection.md`](v2_directive_detection.md). The shipped detector measures **P=0.706 / R=0.937** against lab corpus v0.1 (re-run 2026-08-05, [#1341](https://github.com/robotrocketscience/aelfrice/issues/1341)); the P=0.664 previously quoted here was the pre-Path-A number. **The gate is blocked on the corpus, not the detector** — a first-token-only classifier clears it at P=0.912 / R=0.795 held out, so v0.1 cannot certify a detector. Corpus v0.2 is the blocking item; see that memo § Gate validity.
