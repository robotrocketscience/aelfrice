...
Per [ROADMAP.md § v1.3.0](../concepts/ROADMAP.md), the v1.3 retrieval wave moves
...
[PHILOSOPHY § Determinism is the property](../concepts/PHILOSOPHY.md)) — instead,
...
- **Temporal-coherence fix.** Each hop resolves to the globally
  ...
  for the decision and [LIMITATIONS § BFS multi-hop temporal coherence](../user/LIMITATIONS.md#bfs-multi-hop-temporal-coherence)
...
- **Cross-graph / cross-store walks.** One DB at a time, per
  [LIMITATIONS § Multi-project query](../user/LIMITATIONS.md#multi-project-query).
...
- **Determinism.** All ties (in edge ranking, in result ordering)
  ...
  [PHILOSOPHY § Determinism is the property](../concepts/PHILOSOPHY.md).
...
| `CITES`        | 0.40   | referential    | "B mentions A" — weakest of the explicitly-relational edges. A mention is a pointer, not an argument, so it sits below `SUPPORTS` (0.60) but above the `RELATES_TO` catch-all (0.30). This row previously claimed to *mirror* `EDGE_VALENCE`, where CITES (0.5) is half of SUPPORTS (1.0); it does not — 0.40/0.60 is two thirds — and mirroring would contradict "Why these are not the `EDGE_VALENCE` numbers" below, which is the passage that holds. The two tables are deliberately independent (#1375). |
...
- A new entry in [LIMITATIONS.md § BFS multi-hop temporal
  coherence](../user/LIMITATIONS.md#bfs-multi-hop-temporal-coherence)
...
9. `docs/concepts/ROADMAP.md § v1.3.0` cross-links this spec.
...
- **Temporal coherence.** Resolved above; carry-forward entry in
  [LIMITATIONS.md](../user/LIMITATIONS.md#bfs-multi-hop-temporal-coherence).
  Targeted at v2.0.0.
