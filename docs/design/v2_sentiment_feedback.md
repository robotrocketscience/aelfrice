...
- [ ] **Distribution target.** Apply the signal to *all* previous-turn retrieved beliefs equally, or scale by retrieval rank? (Recommendation: equally — matches the research-line behavior; ranked distribution adds a knob without an evidence-gate.) **Amended #1372 §13:** "equally" was implemented as the *whole* valence per belief, so one utterance wrote N units of evidence over an N-wide pack. It is now an equal *share* — `valence / N` — so one utterance is one unit in total. Rank scaling is still out. This is a magnitude fix only; the share still lands on beliefs the utterance never referred to, and referent-scoped attribution remains open.
