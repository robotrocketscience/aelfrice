...
| rebuilder | `SessionStart(source="compact")` | off | retrieval-curated context rebuilder (augment-mode, v1.4 alpha). The block ships *after* compaction — the `PreCompact` hook does trigger-mode bookkeeping only and injects nothing ([#1031](https://github.com/robotrocketscience/aelfrice/issues/1031)). |
