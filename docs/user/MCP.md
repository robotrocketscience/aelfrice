...
`aelf:stats` and `aelf:health` still emit both the v1.0 schema names (`edges`, `edge_per_belief`) and the v1.1.0 user-facing names (`threads`, `thread_per_belief`) with the same integer value — confirmed in `mcp_server.py:806-807` as of v3.5. The originally-planned v1.2.0 drop did not happen and is no longer scheduled; both forms remain emitted indefinitely. Prefer `threads` / `thread_per_belief` in new code; legacy clients on `edges` continue to work.
