...
Two qualifications, because each has a case where the plain statement is wrong:

- **A pair with equal posteriors never crosses, at any weight.** The blend term is then the same constant for both candidates, so BM25 decides their order forever. This is not a corner case: on a real store, beliefs cluster hard onto a few posterior values, so most candidate pairs are BM25-decided no matter how high you set the weight.
- **"At most once" describes the score expression, not necessarily `retrieve()`.** With `use_intentional_clustering` on — the default — the ranking is produced by a greedy diversity fill over scored candidates rather than by sorting them, and a pack step can in principle move a pair back. Measured on a real store across 40 queries and 121 weights from 0 to 12, no pair crossed twice and no ranking reverted, so the staircase held in practice there; treat single-crossing as a property of the scorer and verify it if you depend on it.

So the failure mode at a high weight is that **the ranking stops responding**, not that it becomes the posterior order: candidates with tied posteriors stay in BM25 order, and only the pairs whose posteriors differ have finished moving. Relatedly, equal metrics between two weights are not evidence the rerank did nothing — aggregates are pooled statistics, and two runs whose rank moves cancel across the corpus report identical numbers while retrieving different rankings.

Where the freeze point falls is a property of your store, not a fixed number, and it is governed mainly by how many **distinct** posterior values your beliefs carry rather than by how many candidates a query returns. A store whose beliefs sit on a handful of posterior values freezes at a low weight however wide you retrieve. The measurements — band edges on the bundled 7-fixture corpus, and a freeze-weight distribution over a real store — are recorded in [#1584](https://github.com/robotrocketscience/aelfrice/issues/1584). Neither is a production constant for your store.

The two alternative rerankers differ here. `use_zeta_posterior_rerank` passes the posterior through a sigmoid, so at a fixed `scale` its contribution is bounded to `[-α·scale/2, +α·scale/2]` and no posterior, however extreme, leverages a belief without limit — but the bound itself grows with `scale`, so a large enough `scale` still reaches a two-level ordering. `use_gamma_posterior_temperature` reparameterises the weight as `1/T` and saturates identically to this key as `T` approaches zero, since `(1/T)·log(p)` is unbounded.
...
- **aelfrice records the exposure, and only the exposure that happened.** The substitution runs before the injection ledger, so an explored belief that reaches the model is recorded as injected like any other hit. A substitution without a record would leave the loop as closed as it was. One asymmetry to know about when you count: the `exploration_events` row is written when the draw is made, while the `injection_events` and `feedback_history` rows are written after the block ceiling has run. The drawn belief sits at the tail of the pack and the ceiling sheds the per-turn tail first, so on a block that overruns, the ledger can name a draw the model never saw. Measured at the shipped ceiling on a 60-lock / 20-core / 12-hit store (`scripts/measure_block_ceiling.py --exploration`), the emitted block was identical with the slot on and off at 5923 estimated tokens while the ledger still recorded one draw and one displacement, 0 of which reached the model. The row is kept deliberately: it is the replay record of the draw, and `derive_seed` is only auditable if every firing turn leaves one.
...
**The default is off, and the correct measurement gates the flip.** This slot is *not* a ranking change, and you must not run an A/B on it as one. Its outcome is the **coverage of the never-injected pool over time**, which you can count from `exploration_events` and `injection_events` without a judge and without a gold set. Count the join, not `drawn_ids` on its own: a draw the block ceiling deleted has a ledger row and no `injection_events` row, so `drawn_ids` alone over-counts coverage. The draw is deliberately **uniform**, because the A-Res weighting the original specification keyed on `scoring.uncertainty_score` is invalid: that function is the Beta *differential* entropy, which is `<= 0` on `[0, 1]`, so the reservoir key divides by zero. After either natural repair of the sign, that weighting is indistinguishable from a uniform draw at a total-variation distance of 0.0586.
