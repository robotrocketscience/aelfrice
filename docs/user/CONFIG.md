...
### `use_answer_worthiness`

Boolean, default `false`. When enabled, the L1 rerank reads each belief's **answer-worthiness** posterior — a query-independent Beta accumulator of the #779 `referenced` signal (`belief_relevance` table) — in place of the belief's own `(α, β)` for the posterior term, but only for beliefs with at least **3** injection observations (`ANSWER_WORTHINESS_MIN_OBS`). Beliefs below that coverage gate cold-start to the type prior, so ranking is unchanged for them. Flag-off is byte-identical to the prior ranking.

The prior is populated passively by the Layer-3 relevance sweeper (it requires the `UserPromptSubmit` hook to be firing, so `injection_events` accumulate). It stays default-off until real accumulated coverage clears the enable gate (`answer_worthiness_coverage()`; target ~20% of retrieved beliefs at/above the min-obs threshold). Basis: the relevance-corpus R&D campaign — a relevance-informative posterior is worth ~+18.6pp recall@1 at the ceiling (R5), robustly learnable under noise/coverage (R6) and structured bias (R7), and generalises across corpora (R8).

Precedence (first decisive wins): env var `AELFRICE_USE_ANSWER_WORTHINESS=0`/`1` > explicit Python kwarg > TOML `[retrieval] use_answer_worthiness` > default `false`. Independent of γ/ζ — it changes the posterior those scorers read, it does not select a scorer.
