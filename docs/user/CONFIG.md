...
**Reachable on `retrieve_v2()` only.** Unlike the four graduated lanes, origin tie-break is not resolver-driven in the production shim: `retrieve()` passes the literal `use_origin_tiebreak=False` rather than `None`, so the resolver is never consulted on that path and **neither the env var nor the kwarg reaches it there**. This is deliberate — it is the one staged rerank lane not graduated in v4.0, and graduating it is a separate operator call, as is the default-ON flip. Note also that a single-provenance corpus (e.g. LoCoMo) shares one origin tier, so the tie-break is inert there regardless.
