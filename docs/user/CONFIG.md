...
Precedence (first decisive wins): env var `AELFRICE_ORIGIN_TIEBREAK=1` > explicit Python kwarg `use_origin_tiebreak=<bool>` > TOML `[retrieval] use_origin_tiebreak` > default `false`. Since the #1107 cutover `retrieve()` is a thin adapter over `retrieve_v2()`, so the flag is honored on both paths — but unlike the four graduated lanes, origin tie-break stays **held off** in the production shim by default (it is the one staged rerank lane not graduated in v4.0). Note a single-provenance corpus (e.g. LoCoMo) shares one origin tier, so the tie-break is inert there; the default-ON flip is a separate operator call.
