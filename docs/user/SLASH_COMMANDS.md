...
| `/aelf:graph` | optional `--belief-id ID`, `--hops N` (default 2), `--format dot\|json`, `--preview-chars N` (default 80) — emits a subgraph with color-coded edges (`SUPERSEDES`, `CONTRADICTS`, `RESOLVES`, `SUPPORTS`, `DERIVED_FROM`) and shaded nodes by origin. v3.0+ ([#629](https://github.com/robotrocketscience/aelfrice/issues/629)). |
| `/aelf:scope-out` | `pattern` (positional) or `--list` / `--clear` — manage the project-scope denylist (federation visibility). v3.0+ ([#856](https://github.com/robotrocketscience/aelfrice/issues/856)). |
| `/aelf:feed` | optional `--limit N`, `--filter EVENT`, `--json` — read the belief-write event log at `<git-common-dir>/aelfrice/feed.jsonl` (lock / onboard / wonder-promote / feedback rows). v3.4+. |
| `/aelf:stale` | `--older-than DAYS`, `--cold-for DAYS` — list beliefs with `created_at` older than N days AND `last_retrieved_at` NULL or older than M days. No decay model; user supplies windows. v3.4+. |
| `/aelf:review` | `--generate` writes `.aelfrice/review.md` with up to 10 oldest-unconfirmed beliefs as a checkbox file; user edits verdicts; subsequent invocation applies keep / remove / lock decisions. v3.5+ ([#936](https://github.com/robotrocketscience/aelfrice/issues/936)). |
...
| `/aelf:audit-claude-memory` | (none) — read-only cross-store dedup audit between locked aelfrice beliefs and the host's `~/.claude/projects/.../memory/MEMORY.md`. Reports potential duplicates, contradictions, and store-exclusive entries. v3.5+ ([#935](https://github.com/robotrocketscience/aelfrice/issues/935)). |
