...
- **Locks are durable.** A fresh lock is `(α, β) = (9.0, 0.5)`. Passive feedback — `aelf feedback <id> harmful`, sentiment-derived turn valence, retrieval exposure, and valence propagation — doesn't move a lock. `apply_feedback` enforces this rule, and the deferred sweeper has enforced the same rule since [#1168](https://github.com/robotrocketscience/aelfrice/issues/1168). `feedback_history` still records the event, and the CLI tells you that it recorded the event but didn't apply it. `aelf confirm` is the exception. Because it's an explicit affirmation rather than passive feedback, `aelf confirm` still moves a locked posterior. Before #1168 the floor lived only in the posterior-decay code, which no module called, so passive feedback did move locks. That code was deleted in [#1369](https://github.com/robotrocketscience/aelfrice/issues/1369); the floor `apply_feedback` enforces is the one that matters. v3.2.0 [#814](https://github.com/robotrocketscience/aelfrice/issues/814) removed the v2.x `demotion_pressure` machinery and the auto-demote machinery; v3.1.0 as released still contained that machinery, see #833. To change a lock, use `aelf unlock` or `aelf delete`, or lock the corrected statement again. `aelf demote <id>` drops the lock immediately.
...
- **A posterior of 0.5 means no evidence, not a coin flip.** Fresh beliefs start at priors that are adjusted for the type and for the source (0.375–0.95), not at 0.5. The Jeffreys prior `(0.5, 0.5)` reads as exactly `0.5`, and that value means "no evidence"; it doesn't mean that the belief is as likely to be true as false. Nothing returns a belief to that value over time: the code that would have aged `(α, β)` back toward the prior had no caller and was deleted in [#1369](https://github.com/robotrocketscience/aelfrice/issues/1369). A belief reading exactly 0.5 therefore carries no net evidence either way; it doesn't mean the evidence was weighed and came out even.
