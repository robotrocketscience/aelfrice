...
Multi-subcommand CLI; full set listed via `aelf --help --advanced`. The retrieval/feedback ones are also exposed as slash commands (see [SLASH_COMMANDS](SLASH_COMMANDS.md)). Most lifecycle commands also ship as slash commands (`setup`, `doctor`, `upgrade`, `uninstall` all have `/aelf:*` forms); only `migrate` and `unsetup` are CLI-only (`aelf unsetup` has no slash form; the `/aelf:uninstall` flow covers teardown).
...
| `retire <belief_id> [--force]` | (v4.0+, [#1081](https://github.com/robotrocketscience/aelfrice/issues/1081)) **Reversible** soft-delete — the gentle sibling of `delete`. Sets `valid_to` so the belief drops out of retrieval and FTS search, while preserving its evidence trail (edges, entity index, corroborations). Writes one `feedback_history` audit row (valence=-1.0, `user_retired`, or `user_retired_force` with `--force`) before the soft-delete. Locked beliefs require `--force`. Retiring an already-retired belief is a no-op. Undo with `restore`. |
| `restore <belief_id>` | (v4.0+, [#1081](https://github.com/robotrocketscience/aelfrice/issues/1081)) Inverse of `retire` / `soft_delete_belief`: clears `valid_to` and re-inserts the pruned FTS row so the belief re-enters keyword search and retrieval. Writes a `user_restored` audit row (valence=+1.0). No-op (reports "not restorable", exits non-zero) on an already-active or unknown id. |
| `introspect [--by session\|project] [--session ID] [--project CTX] [--only-noise] [--limit N] [--json]` | (v4.0+, [#1081](https://github.com/robotrocketscience/aelfrice/issues/1081)) Read-only honest-signal view over active beliefs — the native answer to "analyse the beliefs it extracted." Groups by ingest session (or `--by project`) and surfaces, per belief, the signals the store already holds but never showed together: posterior μ + evidence weight (α+β), **recurrence** (`corroboration_count`, labelled explicitly as recurrence, *not* truth), **grounding** (`durable`/`ephemeral`/`neutral` via `entity_persistence_scores`), floated-vs-decided **status** (from `RESOLVES` / `POTENTIALLY_STALE` edges), and stranded-capture **noise** (`is_stranded_capture_noise`) — noise rows float to the top of each group as the prime retire candidates. `--only-noise` is the retire shortlist; `--limit 0` = no cap. Deterministic per #605 (pure counts + edge lookups, no model, no clock). Curate off it with `retire` / `lock` / `resolve`. |
...
| `confirm <belief_id> [--source S] [--note TEXT]` | Explicit user affirmation: α += 1.0 with source `user_confirmed` (default). Writes to `feedback_history`. Note is printed on success but not persisted. Distinct from `lock` (no freeze) and from implicit retrieval feedback (`used` signal) — and because it is explicit rather than passive, it is the one feedback surface exempt from the [#1168](https://github.com/robotrocketscience/aelfrice/issues/1168) lock floor, so it still moves a locked belief's posterior. |
| `feedback <belief_id> <used\|harmful> [--source S]` | `used` ⇒ α += 1; `harmful` ⇒ β += 1. **No-op on a user lock** ([#1168](https://github.com/robotrocketscience/aelfrice/issues/1168)): passive feedback does not move a lock, so the event is written to `feedback_history`, the posterior is left alone, and the command says so and exits 0. Unlock (or `delete` / `demote`) first to change one. Contradiction-driven lock demotion was removed under [#814](https://github.com/robotrocketscience/aelfrice/issues/814); lock correction now goes through `aelf lock` overwriting per PHILOSOPHY [#605](https://github.com/robotrocketscience/aelfrice/issues/605). |
...
| `category <action>` | (v4.x+, [#1126](https://github.com/robotrocketscience/aelfrice/issues/1126)) Manage keyword-triggered belief categories — named groups of beliefs bound to an activation trigger. Actions: `init` (seed the 5 starter categories idempotently), `add <name> [--always-on] [--keyword PHRASE]… [--tool-glob G]… [--file-glob G]… [--lock none\|locked\|advisory]`, `list`, `show <name>`, `set-trigger <name> [--keyword …]…`, `assign <belief_id> <name>`, `unassign <belief_id> <name>`, `delete <name>`. The `UserPromptSubmit` injection lane that surfaces a fired category's members is **default-off** (`AELFRICE_BELIEF_CATEGORIES=1` or `[belief_categories] enabled=true`) and advisory (never blocks a tool call). Assign at lock time with `lock "<rule>" --category <name>`. Membership is user-driven — no auto-classification. |
...
| `reconcile-claude-memory [--project PATH] [--force]` | (v4.0+, [#1089](https://github.com/robotrocketscience/aelfrice/issues/1089)) Ingest the project's claude-memory fact files (`<memory-dir>/*.md`, excluding `MEMORY.md`) into the belief graph — the full-set sweep that complements the `PostToolUse` write-through mirror (#985). Normally runs once per project at `aelf setup` behind a sentinel; this is the manual re-run. Idempotent — content-derived ids mean a re-run corroborates, not duplicates. `--project` defaults to the current directory; `--force` re-runs past the per-project sentinel. One-way and non-authoritative — aelfrice never writes back to the memory files. |
...
| Command | What it does |
|---|---|
| `setup` | Install the full default hook set: `UserPromptSubmit` retrieval hook + statusline notifier, plus — each default ON with a `--no-*` opt-out — transcript-ingest (four-event loggers), commit-ingest (`PostToolUse:Bash`), session-start (L0 injection), stop-hook (lock prompt), sessionstart-recap, search-tool (`PreToolUse:Grep\|Glob`), search-tool-bash, pre-issue-guard, claude-memory-mirror (`PostToolUse:Write\|Edit\|MultiEdit`, v3.7.0+; runs once the one-shot reconcile at first `aelf setup` records per-project consent, or when `AELFRICE_MIRROR_CLAUDE_MEMORY` / `[memory] mirror_claude_memory` enables it explicitly — an explicit `0`/`false` always wins, #985/#1089), and agent-context (`PreToolUse:Agent`; dispatched workers inherit locked + task-relevant beliefs, #1068). `--rebuilder` is the one opt-in lane; its block ships on `SessionStart(source="compact")`, after compaction, not on `PreCompact` ([#1031](https://github.com/robotrocketscience/aelfrice/issues/1031)). `--host {claude,codex}` selects the host — with `codex`, hooks land in the Codex home's `hooks.json` (`$CODEX_HOME`, else `~/.codex`) and the slash bundle installs as `$aelf-*` agent skills (`--codex-skills` is default-on; `--no-codex-skills` installs hooks only) — see [SLASH_COMMANDS § Codex host](SLASH_COMMANDS.md). Auto-detects scope (`project` if `cwd/.venv` matches the active interpreter, else `user`). Idempotent + atomic. See [INSTALL § default-on hooks](INSTALL.md). |
...
| `rebuild [--transcript PATH] [--n N] [--budget N]` | Manual context-rebuilder run (alpha; automatically fires on `SessionStart(source="compact")`, after compaction — not on `PreCompact`, which cannot inject, [#1031](https://github.com/robotrocketscience/aelfrice/issues/1031)). Prints the rebuild block to stdout. |
